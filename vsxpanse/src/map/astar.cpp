...
#else
static const int DX[DIRECTIONS] = { 1, 1, 0, -1, -1, -1, 0, 1 };
static const int DY[DIRECTIONS] = { 0, 1, 1, 1, 0, -1, -1, -1 };
#endif
...
class AStarNode
{
	...
	*/
	static const int level_weight = 40;
#if (DIRECTIONS == 8)
	static const int diag_level_wight = 80;
#endif
	static const int estimation_weight = 10;
...
bool Astar::find_position_near_position(Vector2DInt target, Vector2DInt * out_position)
{
	/* Just go around and check nearest neighbor */
	for (int dir = 0; dir < DIRECTIONS; ++dir)
	{		
		Vector2DInt new_pos(DX[dir], DY[dir]);
		new_pos += target;
		if(!GridMap::get_if_position_in_gridmap(new_pos)) continue;
		if(!GridMap::get_if_tile_is_free(new_pos)) continue;
...
bool Astar::get_path(Vector2DInt start, Vector2DInt finish, std::stack<Vector2DInt> & path)
{
	start = GridMap::pixelspace_to_gridspace(start);
	finish = GridMap::pixelspace_to_gridspace(finish);
	ASSERT(GridMap::get_if_position_in_gridmap(start), "position not in map");
	ASSERT(GridMap::get_if_position_in_gridmap(finish), "position not in map");
	if (!GridMap::get_if_tile_is_free(finish))
	{
		bool success = find_position_near_position(finish, &finish);
		if (!success)
		{
			Logging::log("Could not find path, the target position was not eligible", Logging::INFO);
			return false;
		}
	}
	...
	while (!node_queue.empty())
	{
		...
		if (steps > 1000)
		{
			Logging::log("Could not find path, over max number of steps", Logging::WARNING);
			return false;
		}
		...
		if (current_node->get_pos() == finish )
		{
			...
			while (curr->get_parent() != nullptr)
			{
				...
				pos = GridMap::gridspace_to_pixelspace(pos);
				path.push(pos);
			...
			for (int dir = 0; dir < DIRECTIONS; ++dir)
			{
				...
				*/
				const bool valid_step =
					GridMap::get_if_position_in_gridmap(new_pos) &&
					GridMap::get_if_tile_is_free(new_pos);
				if (valid_step)
				{
	...
	Logging::log("Could not find path, no possible path", Logging::WARNING);
	return false;
}
